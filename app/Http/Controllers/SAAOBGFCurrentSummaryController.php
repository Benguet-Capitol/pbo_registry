...
use App\Exports\SAAOBGFCurrentSummaryExport;
    ...
    public function index(Request $request)
    {
        ...
        $funds = Fund::where('fund_type', 'General Fund')
            ->with(['officeAllotmentClasses' => function ($query) use ($selectedYear) {
                ...
                ->whereHas('allotmentClass', fn($subQuery) =>
        ...
        foreach ($funds as $fund) {
            ...
            foreach ($grouped as $class => $group) {
                ...
                ->flatMap->obligationAmounts
                ->flatMap(fn($oa) =>
                    $oa->obligationAdjustments
                        ->where('obligation_amounts_id', $oa->id) // ✅ ensure it belongs to this OA
                        ->when($asOfDate, fn($q) => $q->where('adjustment_date', '<=', $asOfDate))
                )
