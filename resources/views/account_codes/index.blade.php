<x-app-layout>
    ...
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6 dark:bg-gray-800">
        <div class="p-6 bg-white rounded-md border-b border-gray-200 relative overflow-x-auto shadow-md sm:rounded-lg dark:bg-gray-800 dark:border-gray-700">
                ...
                <thead class="text-center border-t-2 border-b-2 border-gray-400 text-xs text-gray-700 bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                    <tr>
                        <th class="px-6 py-3 border-gray-300 leading-4 text-gray-600 tracking-wider dark:text-gray-300">
                            <a href="{{ route('account_codes.index', ['sort_by' => 'code', 'sort_order' => $sortBy == 'code' && $sortOrder == 'asc' ? 'desc' : 'asc']) }}">
                                Code
                                @if($sortBy == 'code')
                                {{ $sortOrder == 'asc' ? '▲' : '▼' }}
                                @endif
                        ...
                        <th class="px-6 py-3 border-gray-300 leading-4 text-gray-600 tracking-wider dark:text-gray-300">
                            <a href="{{ route('account_codes.index', ['sort_by' => 'description', 'sort_order' => $sortBy == 'description' && $sortOrder == 'asc' ? 'desc' : 'asc']) }}">
                                Description
                        ...
                        <th class="px-6 py-3 border-gray-300 leading-4 text-gray-600 tracking-wider dark:text-gray-300">
                            <a href="{{ route('account_codes.index', ['sort_by' => 'class', 'sort_order' => $sortBy == 'class' && $sortOrder == 'asc' ? 'desc' : 'asc']) }}">
                                Class
                                @if($sortBy == 'class')
                                {{ $sortOrder == 'asc' ? '▲' : '▼' }}
                                @endif
                        ...
                        @canany(['edit account codes', 'delete account codes'])<th class="px-6 py-3 border-gray-300 leading-4 text-gray-600 tracking-wider dark:text-gray-300">{{ __('Actions') }}</th>@endcanany
                    </tr>
                </thead>
                <tbody id="accountCodesTableBody">
                    @if($accountCodesLoading ?? false)
                        <tr>
                            <td colspan="4" class="px-6 py-6 text-center text-gray-500 dark:text-gray-400 text-sm">
                                <i class="fas fa-spinner fa-spin mr-2"></i>Loading account codes...
                            </td>
                        </tr>
                    @else
                    @foreach ($account_codes as $account_code)
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600">
                        <td class="px-6 py-3 border-b border-gray-300 text-gray-600 dark:text-gray-300">{{ $account_code->code }}</td>
                        <td class="px-6 py-3 border-b border-gray-300 text-gray-600 dark:text-gray-300">{{ $account_code->description }}</td>
                        <td class="px-6 py-3 border-b border-gray-300 text-gray-600 dark:text-gray-300">
                            {{ $account_code->allotmentClass ? $account_code->allotmentClass->description : '-' }}
                        </td>
                        <td class="px-6 py-3 border-b border-gray-300 text-gray-600 dark:text-gray-300">
                            @canany(['edit account codes', 'delete account codes'])
                            <div class="relative inline-block text-left">
                                ...
                                <div class="absolute right-0 mt-1 w-32 bg-white border border-gray-300 rounded-lg shadow-lg hidden dropdown-menu z-10 dark:bg-gray-700 dark:border-gray-600">
                                    @can('edit account codes')
                                    <button onclick='openEditAccountCodeModal(@json($account_code))' class="w-full text-left px-4 py-2 text-xs text-gray-600 hover:bg-gray-200 dark:text-gray-200 dark:hover:bg-gray-600">
                                        <i class="fas fa-edit mr-2"></i>Edit
                                    </button>
                                    @endcan
                                    @can('delete account codes')
                                    <button onclick="openDeleteAccountCodeModal({{ $account_code->id }}, '{{ $account_code->code }}')" class="w-full text-left px-4 py-2 text-xs text-red-600 hover:bg-gray-200 dark:text-red-400 dark:hover:bg-gray-600">
            ...
            <div class="mt-4" id="accountCodesPagination">
                @if ($perPage != 'all')
                {{ $account_codes->appends(request()->query())->links() }}
                @endif
            </div>
        </div>
    </div>

    @include('account_codes.modal.create')
    @include('account_codes.modal.delete')
    @include('account_codes.modal.edit')
...
<script>
    function filterTable() {
        ...
        table = document.getElementById("account_codesTable");
    ...
    function loadAccountCodes(url, pushHistory = true) {
        fetch(url, { headers: { 'X-Requested-With': 'XMLHttpRequest' } })
            .then(response => response.text())
            .then(html => {
                const doc = new DOMParser().parseFromString(html, 'text/html');
                ['accountCodesTableBody', 'accountCodesPagination'].forEach(id => {
            ...
            .catch(error => {
                console.error('Failed to load account codes, falling back to full page navigation:', error);
    ...
    @if($accountCodesLoading ?? false)
        ...
        document.addEventListener('DOMContentLoaded', function() {
            loadAccountCodes(window.location.href, false);
        });
    @endif

    // Delegated click handler for pagination links (they get replaced on every load).
    document.addEventListener('click', function(e) {
        const link = e.target.closest('#accountCodesPagination a');
        if (!link) return;
        e.preventDefault();
        loadAccountCodes(link.href);
    });
</script>
