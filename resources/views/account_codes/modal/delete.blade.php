<div id="deleteModal" style="display: none;" tabindex="-1" aria-labelledby="deleteAccountCodeLabel" aria-hidden="true" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
    <div class="relative w-full max-w-md mx-4 bg-white rounded-lg shadow-lg dark:bg-gray-800 hidden animate-scaleInUp" style="animation: scaleInUp 0.3s ease-out;">
        ...
        <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200 rounded-t-lg bg-gradient-to-r from-red-50 to-pink-50 dark:from-gray-700 dark:to-gray-600 dark:border-gray-600">
            <h3 id="deleteAccountCodeLabel" class="text-base font-semibold text-red-600 dark:text-red-400 flex items-center gap-2">
                <i class="fas fa-exclamation-triangle text-red-600 dark:text-red-400"></i>
                {{ __('Delete Account Code') }}
            </h3>
        ...
        <!-- Modal body -->
        <div class="px-6 py-4">
            <p id="deleteModalContent" class="text-sm text-gray-900 dark:text-gray-200">{{ __('Are you sure you want to delete this Account Code? This action cannot be undone.') }}</p>
        </div>
        <!-- Modal footer -->
...
<script>
    function openDeleteAccountCodeModal(userId, accountCode) {
        closeAllDropdowns();
        const modal = document.getElementById('deleteModal');
        const modalContent = modal.querySelector('div.hidden');
        document.getElementById('deleteForm').action = '{{ route("account_codes.destroy", ":id") }}'.replace(':id', userId);
        document.getElementById('deleteModalContent').innerHTML = `{{ __('Are you sure you want to delete this Account with Account Code:') }} <strong>${accountCode}</strong>? {{ __('This action cannot be undone.') }}`;
