<div id="deleteModal" class="fixed inset-0 z-50 bg-gray-900 bg-opacity-50 overflow-y-auto h-full w-full hidden flex items-center justify-center transition-all duration-300 ease-in-out animate-fadeIn p-4" style="display: none;">
    <div class="flex items-center justify-center">
        <div class="relative w-full max-w-md p-0 bg-white rounded-xl shadow-2xl border-2 border-red-200 dark:bg-gray-800 dark:border-red-900 transform transition-all duration-300 ease-out animate-scaleInUp">
            ...
            <div class="py-6 px-6">
                <p id="deleteModalContent" class="text-gray-700 dark:text-gray-300 leading-relaxed">{{ __('Are you sure you want to delete this Account? This action cannot be undone.') }}</p>
            </div>
...
<script>
    function openDeleteModal(appropriationId, appropriation, accountCode, description) {
        ...
        Are you sure you want to delete the Account with <strong class="font-semibold text-red-600 dark:text-red-400">${formattedAppropriation}</strong> 
        under <strong class="font-semibold text-gray-900 dark:text-gray-200">${accountCode} - ${description}</strong>?
