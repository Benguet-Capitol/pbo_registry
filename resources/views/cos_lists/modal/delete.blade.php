...
<div id="deleteModal" style="display: none;" aria-hidden="true" class="fixed inset-0 z-[10002] flex items-center justify-center bg-black bg-opacity-50">
    <div class="relative w-full max-w-md mx-4 bg-white rounded-lg shadow-lg dark:bg-gray-800 animate-scaleInUp" style="animation: scaleInUp 0.3s ease-out;">
        ...
        <!-- Modal body -->
        <div class="px-6 py-4">
            <p id="deleteModalContent" class="text-gray-700 dark:text-gray-200 text-sm">{{ __('Are you sure you want to delete this Contract of Service? This action cannot be undone.') }}</p>
        </div>
        <!-- Modal footer -->
...
<script>
    function openDeleteModal(cosId, employeeName, officeAbbreviation, allotmentClass, annualRate) {
        closeAllDropdowns();
        // Ensure annual rate is a valid number before formatting
        let formattedAnnualRate = "0.00"; // Default value

        if (!isNaN(annualRate) && annualRate !== null && annualRate !== "") {
            formattedAnnualRate = parseFloat(annualRate).toLocaleString('en-US', {
                minimumFractionDigits: 2,
                maximumFractionDigits: 2
            });
        }

        document.getElementById('deleteForm').action = '{{ route("cos_lists.destroy", ":id") }}'.replace(':id', cosId);
        document.getElementById('deleteModalContent').innerHTML = `Are you sure you want to delete this Contract of Service for <strong>${employeeName}</strong>? <br> This action cannot be undone.`;
