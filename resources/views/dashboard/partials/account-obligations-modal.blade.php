{{-- Obligations Modal (accounts page): listing modal, its row context menu, the
     obligation history modal, and the cancellation modal, plus all of their JS.
     Extracted from dashboard/accounts.blade.php to keep that file's length manageable. --}}

    <!-- Obligations Modal for Accounts -->
    <div id="accountObligationsModal" style="display: none;" aria-hidden="true" class="fixed inset-0 z-[10000] flex items-center justify-center bg-black bg-opacity-50 p-2 sm:p-4">
        <div class="flex flex-col max-h-[95vh] sm:max-h-[90vh] w-full max-w-screen-2xl mx-auto bg-white dark:bg-gray-800 rounded-lg shadow-2xl animate-scaleInUp">
            ...
            <div class="flex justify-between items-start sm:items-center gap-2 px-4 sm:px-6 py-3 sm:py-4 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900 dark:to-indigo-900 border-b-2 border-blue-200 dark:border-blue-700 rounded-t-lg">
                <div class="flex items-start sm:items-center gap-2 sm:gap-3 min-w-0">
                    ...
                    <div class="min-w-0">
                        ...
                        <span id="accountObligationsHeaderInfo" class="block text-[11px] sm:text-xs text-blue-700 dark:text-blue-300 break-words"></span>
                    </div>
                </div>
                <button onclick="closeAccountObligationsModal()" class="flex-shrink-0 text-blue-600 dark:text-blue-300 hover:text-white hover:bg-blue-600 dark:hover:bg-blue-700 rounded-full p-2 transition-colors duration-200">
            ...
            <div class="px-4 sm:px-6 py-3 border-b border-gray-200 dark:border-gray-700 bg-blue-50/50 dark:bg-blue-950/20 space-y-2.5">
                ...
                <div class="flex items-center justify-between gap-2">
                    <div class="flex items-center space-x-2 px-3 sm:px-4 py-2 bg-blue-50 dark:bg-gray-700 rounded-lg border border-blue-200 dark:border-gray-600 flex-shrink-0">
                        ...
                        <span id="accountObligationsTotalRecordsOfWrap" class="hidden text-xs text-blue-500 dark:text-blue-400">of <span id="accountObligationsTotalRecordsAll">0</span></span>
                    </div>
                    <!-- Clear All Filters -->
                    <button
                        type="button"
                        id="accountObligationsClearFiltersBtn"
                        onclick="clearObligationsFilters('accounts')"
                    ...
                    >
                        ...
                        <span id="accountObligationsActiveFilterCount" class="inline-flex items-center justify-center w-4 h-4 text-[10px] rounded-full bg-red-600 text-white">0</span>
                    </button>
                </div>
                ...
                <div class="flex flex-wrap items-center gap-2 pt-2.5 border-t border-blue-100 dark:border-blue-900">
                    ...
                    <div class="flex items-center gap-2 flex-wrap">
                        ...
                        <select
                            id="accountObligationsDatePreset"
                            onchange="applyObligationsDatePreset(this.value, 'accounts')"
                        ...
                        <input
                            ...
                            onchange="document.getElementById('accountObligationsDatePreset').value = ''; refreshAccountObligationsModal(); updateObligationsFilterIndicator('accounts')"
                        >
                    ...
                    <div class="relative w-full sm:flex-1 sm:min-w-[180px]">
                        ...
                        <input
                            ...
                            oninput="filterObligationsTable(this.value, 'accounts'); toggleObligationsSearchClear(this.value, 'accounts'); updateObligationsFilterIndicator('accounts')"
                        >
                        <i id="accountObligationsSearchSpinner" class="hidden fas fa-circle-notch fa-spin absolute right-8 top-1/2 -translate-y-1/2 text-blue-400 text-xs"></i>
                        <button
                            type="button"
                            id="accountObligationsSearchClearBtn"
                            onclick="clearObligationsSearch('accounts')"
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-3 p-3 sm:p-6 border-t-2 border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-800 rounded-b-lg flex-shrink-0">
                <span id="accountObligationsExportHint" class="text-[11px] text-gray-500 dark:text-gray-400 text-center sm:text-left order-2 sm:order-1"></span>
                <div class="flex flex-col sm:flex-row gap-2 sm:gap-3 order-1 sm:order-2">
                    <button id="accountObligationsExportBtn" onclick="exportObligationsModal('accounts')" disabled class="w-full sm:w-auto justify-center bg-green-600 dark:bg-green-600 text-white inline-flex leading-4 tracking-wider items-center hover:bg-green-700 dark:hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-green-600 text-xs px-6 py-2 transition-all duration-200 ease-in-out transform hover:scale-105 active:scale-95 disabled:hover:scale-100 disabled:active:scale-100 rounded-lg">
                        <i id="accountObligationsExportIcon" class="fas fa-file-excel mr-2"></i>
                        <span id="accountObligationsExportLabel">Export</span>
                    </button>
                    <button onclick="closeAccountObligationsModal()" class="w-full sm:w-auto justify-center text-gray-600 dark:text-gray-300 inline-flex leading-4 tracking-wider items-center hover:text-white border border-gray-600 dark:border-gray-400 hover:bg-gray-600 dark:hover:bg-gray-600 text-xs px-6 py-2 transition-all duration-200 ease-in-out transform hover:scale-105 active:scale-95 rounded-lg">
    ...
    <div id="accountObligationsContextMenu" class="hidden fixed w-48 max-w-[calc(100vw-1rem)] bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-900 dark:to-blue-800 border-2 border-blue-400 dark:border-blue-600 rounded-lg shadow-2xl" style="z-index: 10001; position: fixed;">
    ...
    <form id="accountsCancelObligationForm" method="POST">
        <div id="accountsCancellationModal" style="display: none;" aria-hidden="true" class="fixed inset-0 z-[10004] flex items-center justify-center bg-black bg-opacity-50 p-2 sm:p-4">
            <div class="flex flex-col max-h-[95vh] sm:max-h-[90vh] w-full max-w-2xl mx-auto bg-white dark:bg-gray-800 rounded-lg shadow-2xl animate-scaleInUp">
                ...
                <div class="flex justify-between items-center px-4 sm:px-6 py-3 sm:py-4 bg-gradient-to-r from-purple-50 to-violet-50 dark:from-purple-900 dark:to-violet-900 border-b-2 border-purple-200 dark:border-purple-700 rounded-t-lg">
                    ...
                    <button type="button" onclick="closeAccountsCancellationModal()" class="flex-shrink-0 text-purple-600 dark:text-purple-300 hover:text-white hover:bg-purple-600 dark:hover:bg-purple-700 rounded-full p-2 transition-colors duration-200">
                        <i class="fas fa-times text-lg sm:text-xl"></i>
                    </button>
                </div>
                <!-- Modal body (scrollable) -->
    ...
    function showAccountObligationsModal() {
        ...
        headerInfo.innerHTML = ' | <span class="font-semibold">' + (currentAccountAppropriation.accountCode || 'N/A') + ' - ' + (currentAccountAppropriation.description || 'N/A') + '</span>';
        if (loading) loading.style.display = 'flex';
        const acctSearchSpinner = document.getElementById('accountObligationsSearchSpinner');
        if (acctSearchSpinner) acctSearchSpinner.classList.remove('hidden');
        ...
        fetch(apiUrl)
            ...
            .then(data => {
                ...
                const acctSearchSpinnerDone = document.getElementById('accountObligationsSearchSpinner');
                if (acctSearchSpinnerDone) acctSearchSpinnerDone.classList.add('hidden');
                ...
                if (data.success) {
                    ...
                    headerInfo.innerHTML = ` | <span class="font-semibold">${currentAccountAppropriation.accountCode} - ${currentAccountAppropriation.description}</span> | Office: ${data.office || '-'} - Class: ${data.allotmentClass || '-'} (CY ${data.cy_year || '-'})`;
                }
                ...
                if (data.success && data.data && Array.isArray(data.data) && data.data.length > 0) {
                    ...
                    data.data.forEach((obligation, index) => {
                        ...
                        const amountDisplay = isCancelled ?
                            '<span class="bg-red-100 text-red-700 dark:bg-red-900 dark:text-red-400 px-2 py-1 rounded font-semibold">Cancelled</span>' :
                            formattedOblAmount;

                        const rowBandClass = index % 2 === 0 ? 'bg-white dark:bg-gray-800' : 'bg-gray-50 dark:bg-gray-900/40';

                        tableHTML += `
                            <tr class="${rowBandClass} hover:bg-blue-50 dark:hover:bg-blue-900/20 cursor-pointer obligation-row" data-obligation-index="${index}">
                        ...
                        obligation.appropriations.forEach((app, appIndex) => {
                            ...
                            tableHTML += `
                                                <tr class="${appRowBand}">
                                                    <td class="px-3 py-2 border-l-4 border-l-blue-500">${app.programs}</td>
                    ...
                    if (content) {
                        ...
                        const acctOfWrap = document.getElementById('accountObligationsTotalRecordsOfWrap');
                        if (acctOfWrap) acctOfWrap.classList.add('hidden');
                        ...
                        if (typeof updateObligationsFilterIndicator === 'function') {
                            updateObligationsFilterIndicator('accounts');
                        }
                    }
                ...
                updateObligationsExportState('accounts');
            })
            .catch(error => {
                if (loading) loading.style.display = 'none';
                const acctSearchSpinnerErr = document.getElementById('accountObligationsSearchSpinner');
                if (acctSearchSpinnerErr) acctSearchSpinnerErr.classList.add('hidden');
                if (content) {
    ...
    async function exportObligationsModal(source = 'dashboard') {
        ...
        // Remove all hidden obligation rows (by search) and any rows outside the
        // selected date range, and their corresponding appropriations rows
        ...
        // Remove hidden/out-of-range rows from the DOM
    ...
    // Close the Account Obligations modal on Escape, matching the "Esc" hint on its Close button
    document.addEventListener('keydown', function(e) {
        if (e.key !== 'Escape') return;
        const modal = document.getElementById('accountObligationsModal');
        if (modal && modal.style.display !== 'none') {
            closeAccountObligationsModal();
        }
    });
    </script>
