<div id="deleteDisbursementModal" class="fixed inset-0 z-50 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full hidden">
    <div class="flex items-center justify-center min-h-screen px-4">
        <div class="relative w-full max-w-md p-4 bg-white rounded-lg shadow-lg border border-gray-300 dark:bg-gray-800 dark:border-gray-700">
            <div class="flex justify-between items-center pb-3">
                <h3 class="text-lg font-semibold text-red-600 dark:text-white">{{ __('Delete Disbursement') }}</h3>
                <button onclick="closeDeleteDisbursementModal()" class="text-gray-400 hover:text-gray-600 dark:text-gray-200 dark:hover:text-gray-400">
                    <i class="fas fa-times h-6 w-6"></i>
                </button>
            </div>
            <div class="py-4">
                <p id="deleteDisbursementModalContent" class="text-gray-700 dark:text-gray-200">{{ __('Are you sure you want to delete this Disbursement? This action cannot be undone.') }}</p>
            </div>
...
<script>
    ...
    document.getElementById('deleteDisbursementModal').classList.remove('hidden');
    }

    function closeDeleteDisbursementModal() {
        document.getElementById('deleteDisbursementModal').classList.add('hidden');
    }
</script>