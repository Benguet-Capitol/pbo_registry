<div id="deleteModal" style="display: none;" class="fixed inset-0 z-50 bg-gray-600 bg-opacity-50 flex items-center justify-center">
    <div class="w-full max-w-md rounded-xl shadow-2xl transform transition-all duration-300 ease-out bg-white dark:bg-gray-800 overflow-hidden hidden animate-scaleInUp" style="animation: scaleInUp 0.3s ease-out;">
        <div class="flex justify-between items-center p-6 border-b-2 border-red-200 dark:border-red-900 bg-gradient-to-r from-red-50 to-pink-50 dark:from-gray-700 dark:to-gray-700 rounded-t-xl flex-shrink-0">
        ...
        <div class="py-6 px-6">
            <p id="deleteModalContent" class="text-gray-700 dark:text-gray-300 leading-relaxed">{{ __('Are you sure you want to delete this employee? This action cannot be undone.') }}</p>
        </div>
...
<script>
    function openDeleteModal(employeeId, employeeName, employeeOffice) {
        closeAllDropdowns();
        const modal = document.getElementById('deleteModal');
        
        // Update form action and content
        document.getElementById('deleteForm').action = `/employees/${employeeId}`;
        document.getElementById('deleteModalContent').innerHTML = `Are you sure you want to delete the employee <strong>${employeeName}</strong> from office <strong>${employeeOffice}</strong>? This action cannot be undone.`;
