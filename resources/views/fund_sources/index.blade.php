        ...
        <div class="p-6 bg-white rounded-md border-b border-gray-200 relative overflow-x-auto shadow-md sm:rounded-lg dark:bg-gray-800 dark:border-gray-700">
            <div class="flex flex-col lg:flex-row lg:justify-between lg:items-center mb-4 gap-4">
                ...
                <div class="flex flex-col sm:flex-row sm:items-center gap-2 w-full sm:w-auto">
                    <x-form.input type="text" name="search" id="searchInput" value="{{ request('search') }}" autocomplete="off" placeholder="Search for fund sources" class="border border-gray-300 w-full sm:w-[300px] min-w-0 rounded-lg px-4 py-2 text-xs dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white" />
                    <form method="GET" action="{{ route('fund_sources.index') }}" class="w-full sm:w-auto">
                ...
                <thead class="text-center border-t-2 border-b-2 border-gray-400 text-xs text-gray-700 bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                    <tr>
                        <th class="px-6 py-3 border-gray-300 leading-4 text-gray-600 tracking-wider dark:text-gray-300">
                            <a href="{{ route('fund_sources.index', ['sort_by' => 'source', 'sort_order' => $sortBy == 'source' && $sortOrder == 'asc' ? 'desc' : 'asc']) }}">
                                Source
                                @if($sortBy == 'source')
                                {{ $sortOrder == 'asc' ? '▲' : '▼' }}
                                @endif
                        ...
                        <th class="px-6 py-3 border-gray-300 leading-4 text-gray-600 tracking-wider dark:text-gray-300">
                            <a href="{{ route('fund_sources.index', ['sort_by' => 'category', 'sort_order' => $sortBy == 'category' && $sortOrder == 'asc' ? 'desc' : 'asc']) }}">
                                Category
                                @if($sortBy == 'category')
                                {{ $sortOrder == 'asc' ? '▲' : '▼' }}
                                @endif
                        ...
                        @canany(['edit fund sources', 'delete fund sources'])<th class="px-6 py-3 border-gray-300 leading-4 text-gray-600 tracking-wider dark:text-gray-300">{{ __('Action') }}</th>@endcanany
                    </tr>
                </thead>
                <tbody id="fundSourcesTableBody">
                    @if($fundSourcesLoading ?? false)
                        <tr>
                            <td colspan="3" class="px-6 py-6 text-center text-gray-500 dark:text-gray-400 text-sm">
                                <i class="fas fa-spinner fa-spin mr-2"></i>Loading fund sources...
                            </td>
                        </tr>
                    @else
                    @foreach ($fund_sources as $fund_source)
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600">
                        <td class="px-6 py-3 border-b border-gray-300 text-gray-600 dark:text-gray-300">{{ $fund_source->source }}</td>
                        <td class="px-6 py-3 border-b border-gray-300 text-gray-600 dark:text-gray-300">{{ $fund_source->category }}</td>
                        <td class="px-6 py-3 border-b border-gray-300 text-gray-600 dark:text-gray-300">
                            @canany(['edit fund sources', 'delete fund sources'])
                            <div class="relative inline-block text-left">
                                ...
                                <div class="absolute right-0 mt-1 w-32 bg-white border border-gray-300 rounded-lg shadow-lg hidden dropdown-menu z-10 dark:bg-gray-700 dark:border-gray-600">
                                    @can('edit fund sources')
                                    <button onclick='openEditFundSourceModal(@json($fund_source))' class="w-full text-left px-4 py-2 text-xs text-gray-600 hover:bg-gray-200 dark:text-gray-200 dark:hover:bg-gray-600">
                                        <i class="fas fa-edit mr-2"></i>Edit
                                    </button>
                                    @endcan
                                    @can('delete fund sources')
                                    <button onclick="openDeleteFundSourceModal({{ $fund_source->id }}, '{{ $fund_source->source }}')" class="w-full text-left px-4 py-2 text-xs text-red-600 hover:bg-gray-200 dark:text-red-400 dark:hover:bg-gray-600">
            ...
            <div class="mt-4" id="fundSourcesPagination">
                @if ($perPage != 'all')
                {{ $fund_sources->appends(request()->query())->links() }}
                @endif
            </div>
        </div>
    </div>

    @include('fund_sources.modal.create')
    @include('fund_sources.modal.edit')
    @include('fund_sources.modal.delete')
...
<script>
    ...
    function loadFundSources(url, pushHistory = true) {
        fetch(url, { headers: { 'X-Requested-With': 'XMLHttpRequest' } })
            .then(response => response.text())
            .then(html => {
                const doc = new DOMParser().parseFromString(html, 'text/html');
                ['fundSourcesTableBody', 'fundSourcesPagination'].forEach(id => {
            ...
            .catch(error => {
                console.error('Failed to load fund sources, falling back to full page navigation:', error);
    ...
    @if($fundSourcesLoading ?? false)
        ...
        document.addEventListener('DOMContentLoaded', function() {
            loadFundSources(window.location.href, false);
        });
    @endif

    // Delegated click handler for pagination links (they get replaced on every load).
    document.addEventListener('click', function(e) {
        const link = e.target.closest('#fundSourcesPagination a');
        if (!link) return;
        e.preventDefault();
        loadFundSources(link.href);
    });
</script>
