    ...
    <div class="flex items-center justify-center min-h-screen px-4">
        <div class="relative w-full max-w-md p-4 bg-white rounded-lg shadow-lg border border-gray-300 dark:bg-gray-800 dark:border-gray-700">
            <div class="flex justify-between items-center pb-3">
                <h3 class="text-lg font-semibold text-red-600 dark:text-white">{{ __('Delete Fund Source') }}</h3>
            ...
            <div class="py-4">
                <p id="deleteModalContent" class="text-gray-900 dark:text-gray-200">{{ __('Are you sure you want to delete this Fund Source? This action cannot be undone.') }}</p>
            </div>
...
<script>
    function openDeleteFundSourceModal(userId, fundSource) {
        closeAllDropdowns();
        document.getElementById('deleteForm').action = '{{ route("fund_sources.destroy", ":id") }}'.replace(':id', userId);
        document.getElementById('deleteModalContent').innerHTML = `Are you sure you want to delete the Fund Source: <strong>${fundSource}</strong>? This action cannot be undone.`;
