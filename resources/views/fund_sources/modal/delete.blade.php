<div id="deleteModal" style="display: none;" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
    <div class="relative w-full max-w-md mx-4 bg-white rounded-lg shadow-lg dark:bg-gray-800 hidden animate-scaleInUp" style="animation: scaleInUp 0.3s ease-out;">
        ...
        <!-- Modal body -->
        <div class="px-6 py-4">
            <p id="deleteModalContent" class="text-sm text-gray-900 dark:text-gray-200">{{ __('Are you sure you want to delete this Fund Source? This action cannot be undone.') }}</p>
        </div>
        <!-- Modal footer -->
...
<script>
    function openDeleteFundSourceModal(userId, fundSource) {
        ...
        document.getElementById('deleteModalContent').innerHTML = `{{ __('Are you sure you want to delete the Fund Source: ') }}<strong>${fundSource}</strong>? {{ __('This action cannot be undone.') }}`;
