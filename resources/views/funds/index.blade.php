<x-app-layout>
        ...
        <div class="p-6 bg-white rounded-md border-b border-gray-200 relative overflow-x-auto shadow-md sm:rounded-lg dark:bg-gray-800 dark:border-gray-700">
            <div class="flex justify-between items-center mb-4">
                ...
                <div class="flex items-center">
                    <div class="flex items-center">
                        <x-form.input type="text" name="search" id="searchInput" value="{{ request('search') }}" autocomplete="off" placeholder="Search for funds" class="border border-gray-300 w-[300px] rounded-lg px-4 py-2 mr-2 text-xs dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white" />
            ...
            <table id="fundsTable" class="text-center w-full text-xs text-left rtl:text-right text-gray-500 dark:text-gray-400">
                <thead class="text-center border-t-2 border-b-2 border-gray-400 text-xs text-gray-700 bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                    <tr>
                        <th class="px-6 py-3 border-gray-300 leading-4 text-gray-600 tracking-wider dark:text-gray-300">
                            <a href="{{ route('funds.index', ['sort_by' => 'fund', 'sort_order' => $sortBy == 'fund' && $sortOrder == 'asc' ? 'desc' : 'asc']) }}">
                                Fund
                                @if($sortBy == 'fund')
                                {{ $sortOrder == 'asc' ? '▲' : '▼' }}
                                @endif
                        ...
                        <th class="px-6 py-3 border-gray-300 leading-4 text-gray-600 tracking-wider dark:text-gray-300">
                            <a href="{{ route('funds.index', ['sort_by' => 'fund_type', 'sort_order' => $sortBy == 'fund_type' && $sortOrder == 'asc' ? 'desc' : 'asc']) }}">
                                Fund Type
                                @if($sortBy == 'fund_type')
                                {{ $sortOrder == 'asc' ? '▲' : '▼' }}
                                @endif
                        ...
                        <th class="px-6 py-3 border-gray-300 leading-4 text-gray-600 tracking-wider dark:text-gray-300">
                            <a href="{{ route('funds.index', ['sort_by' => 'fund_code', 'sort_order' => $sortBy == 'fund_code' && $sortOrder == 'asc' ? 'desc' : 'asc']) }}">
                                Fund Code
                                @if($sortBy == 'fund_code')
                                {{ $sortOrder == 'asc' ? '▲' : '▼' }}
                                @endif
                ...
                <tbody>
                    ...
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600">
                        ...
                        <td class="px-6 py-3 border-b border-gray-300 text-gray-600 dark:text-gray-300">{{ $fund->fund_type }}</td>
                        <td class="px-6 py-3 border-b border-gray-300 text-gray-600 dark:text-gray-300">{{ $fund->fund_code }}</td>
                        <td class="px-6 py-3 border-b border-gray-300 text-gray-600 dark:text-gray-300">
                            @canany(['edit funds', 'delete funds'])
                            <div class="relative inline-block text-left">
                                ...
                                <div class="absolute right-0 mt-1 w-32 bg-white border border-gray-300 rounded-lg shadow-lg hidden dropdown-menu z-10 dark:bg-gray-700 dark:border-gray-600">
                                    @can('edit funds')
                                    <button onclick='openEditFundModal(@json($fund))' class="w-full text-left px-4 py-2 text-xs text-gray-600 hover:bg-gray-200 dark:text-gray-200 dark:hover:bg-gray-600">
                                        <i class="fas fa-edit mr-2"></i>Edit
                                    </button>
                                    @endcan
                                    @can('delete funds')
                                    <button onclick="openDeleteFundModal({{ $fund->id }}, '{{ $fund->fund }}')" class="w-full text-left px-4 py-2 text-xs text-red-600 hover:bg-gray-200 dark:text-red-400 dark:hover:bg-gray-600">
    ...
    @include('funds.modal.create')
    @include('funds.modal.edit')
    @include('funds.modal.delete')
...
<script>
    function filterTable() {
        ...
        table = document.getElementById("fundsTable");
