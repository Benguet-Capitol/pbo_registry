    ...
    <div id="createFundModal" style="display: none;" tabindex="-1" aria-labelledby="createFundLabel" aria-hidden="true" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
        <div class="relative w-full max-w-4xl mx-4 bg-white rounded-lg shadow-lg dark:bg-gray-800 hidden animate-scaleInUp" style="animation: scaleInUp 0.3s ease-out;">
            ...
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200 rounded-t-lg bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-gray-700 dark:to-gray-600 dark:border-gray-600">
                <h3 id="createFundLabel" class="text-base font-semibold text-gray-900 dark:text-white flex items-center gap-2">
            ...
            <div class="px-6 py-4 overflow-y-auto" style="max-height: calc(90vh - 200px);">
                    <div class="grid gap-4">
                        <!-- Fund -->
                        <div class="space-y-2">
                            <x-form.label for="fund" :value="__('Fund')" class="block text-xs font-medium text-gray-900 dark:text-gray-200" />
                            <x-form.input-with-icon-wrapper>
                                <x-slot name="icon">
                                    <i class="fas fa-star-of-david"></i>
                                </x-slot>
                                <x-form.input withicon id="fund" class="block w-full text-xs dark:bg-gray-800 dark:text-gray-200" type="text" name="fund" :value="old('fund')" autocomplete="off" autofocus placeholder="{{ __('Fund') }}" />
                            </x-form.input-with-icon-wrapper>
                            <span id="fundError" class="text-red-500 text-xs"></span>
                        </div>
                        <!-- Fund Type -->
                        <div class="space-y-2">
                            <x-form.label for="fund_type" :value="__('Fund Type')" class="block text-xs font-medium text-gray-900 dark:text-gray-200" />
                            <x-form.input-with-icon-wrapper>
                                <x-slot name="icon">
                                    <i class="fas fa-spinner"></i>
                                </x-slot>
                                <x-form.select withicon id="fund_type" class="block w-full text-xs dark:bg-gray-800 dark:text-gray-200" type="text" name="fund_type" :value="old('fund_type')" placeholder="{{ __('Fund Type') }}">
                            ...
                            <span id="fund_typeError" class="text-red-500 text-xs"></span>
                        </div>
                        <!-- Fund Code -->
                        <div class="space-y-2">
                            <x-form.label for="fund_code" :value="__('Fund Code')" class="block text-xs font-medium text-gray-900 dark:text-gray-200" />
                            <x-form.input-with-icon-wrapper>
                                <x-slot name="icon">
                                    <i class="fas fa-sticky-note"></i>
                                </x-slot>
                                <x-form.input withicon id="fund_code" class="block w-full text-xs dark:bg-gray-800 dark:text-gray-200" type="text" name="fund_code" autocomplete="off" :value="old('fund_code')" autofocus placeholder="{{ __('Fund Code') }}" />
                            </x-form.input-with-icon-wrapper>
                            <span id="fund_codeError" class="text-red-500 text-xs"></span>
                        </div>
                    </div>
                </div>
                <!-- Modal footer -->
...
<script>
    ...
    function validateCreateFundForm() {
        if (isSubmittingFund) return false;
