    ...
    <div class="flex items-center justify-center min-h-screen px-4">
        <div class="relative w-full max-w-md p-4 bg-white rounded-lg shadow-lg border border-gray-300 dark:bg-gray-800 dark:border-gray-700">
            <div class="flex justify-between items-center pb-3">
                <h3 class="text-lg font-semibold text-red-600 dark:text-white">{{ __('Delete Obligation Request') }}</h3>
            ...
            <div class="py-4">
                <p id="deleteModalContent" class="text-gray-700 dark:text-gray-200">{{ __('Are you sure you want to delete this Obligation Request? This action cannot be undone.') }}</p>
            </div>
...
<script>
    function openDeleteModal(userId, obrNo, officeAbbreviation, Class, obrAmount) {
        ...
        document.getElementById('deleteModalContent').innerHTML = `Are you sure you want to delete this Obligation Request with No. <strong>${obrNo}</strong> under <strong>${officeAbbreviation} - ${Class}</strong> with Total Amount: <strong>${formattedObrAmount}</strong>? This action cannot be undone.`;
