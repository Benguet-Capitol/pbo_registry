...
<div id="obligationModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full hidden">
...
<script>
                ...
                document.getElementById('modalContent').innerHTML = `
                    ...
                    <div class="mt-2">
                        <table class="w-full text-xs text-center border-t mt-3 dark:text-gray-400">
                            ...
                            <tbody>
                                ${obligation_amounts.map((oa, idx) => {
        // Find adjustments for this obligation amount (by account_code and description)
        const relatedAdjustments = obligation_adjustments.filter(adj =>
            adj.account_code == oa.account_code && adj.description == oa.description
        );
        // Find purchase orders for this obligation amount (by account_code and description)
        const relatedPOs = purchase_orders.filter(po =>
            po.account_code == oa.account_code && po.description == oa.description
        );
        const rowId = `oa-details-${idx}`;
        let oaRow = `<tr class=\"bg-white border-b dark:bg-gray-800 dark:border-gray-700 cursor-pointer\" onclick=\"toggleDetails('${rowId}')\">
            <td class=\"px-3 py-2\">${oa.programs || ''}</td>
            <td class=\"px-3 py-2\">${oa.account_code || 'N/A'}</td>
            <td class=\"px-3 py-2\">${oa.description || 'N/A'}</td>
            <td class=\"px-3 py-2 text-right\">${formatCurrency(oa.obr_amount)}</td>
            <td class=\"px-3 py-2 text-right\">${formatCurrency(oa.adjustments)}</td>
            <td class=\"px-3 py-2 text-right\">${formatCurrency(parseFloat(oa.obr_amount) + parseFloat(oa.adjustments || 0))}</td>
            ${showPO ? `<td class=\"px-3 py-2 text-right\">${formatCurrency(oa.po_total)}</td>` : ''}
        </tr>`;
        // Adjustment and PO details (hidden by default)
        let detailsRows = `<tbody id=\"${rowId}\" style=\"display:none\">`;
        // Adjustment label row
        if (relatedAdjustments.length) {
            detailsRows += `<tr class=\"bg-gray-50 dark:bg-gray-900 border-b dark:border-gray-700\">
                <td colspan=\"${showPO ? 7 : 6}\" class=\"font-semibold text-xs px-3 py-2 text-gray-600 dark:text-gray-300 text-left\">Adjustment(s):</td>
            </tr>
            <tr class=\"bg-gray-50 dark:bg-gray-900 border-b dark:border-gray-700\">
                <td class=\"font-semibold text-xs px-3 py-2 text-gray-600 dark:text-gray-300\">Date</td>
                <td class=\"font-semibold text-xs px-3 py-2 text-gray-600 dark:text-gray-300\">Amount</td>
                <td class=\"font-semibold text-xs px-3 py-2 text-gray-600 dark:text-gray-300\">Remarks</td>
                <td colspan=\"${showPO ? 4 : 3}\"></td>
            </tr>`;
            detailsRows += relatedAdjustments.map(adj => `
                <tr class=\"bg-gray-50 dark:bg-gray-900 border-b dark:border-gray-700\">
                    <td class=\"text-center px-3 py-2 text-xs text-gray-500\">${adj.adjustment_date || ''}</td>
                    <td class=\"text-right px-3 py-2 text-xs text-gray-500\">${formatCurrency(adj.adjustment_amount)}</td>
                    <td class=\"text-center px-3 py-2 text-xs text-gray-500\">${adj.remarks || ''}</td>
                    <td colspan=\"${showPO ? 4 : 3}\"></td>
                </tr>
            `).join('');
        }
        // Purchase Order label row
        if (showPO && relatedPOs.length) {
            detailsRows += `<tr class=\"bg-gray-50 dark:bg-gray-900 border-b dark:border-gray-700\">
                <td colspan=\"${showPO ? 7 : 6}\" class=\"font-semibold text-xs px-3 py-2 text-gray-600 dark:text-gray-300 text-left\">Purchase Order(s):</td>
            </tr>
            <tr class=\"bg-gray-50 dark:bg-gray-900 border-b dark:border-gray-700\">
                <td class=\"font-semibold text-xs px-3 py-2 text-gray-600 dark:text-gray-300\">PO Number</td>
                <td class=\"font-semibold text-xs px-3 py-2 text-gray-600 dark:text-gray-300\">PO Date</td>
                <td class=\"font-semibold text-xs px-3 py-2 text-gray-600 dark:text-gray-300\">Supplier</td>
                <td class=\"font-semibold text-xs px-3 py-2 text-gray-600 dark:text-gray-300\">Amount</td>
                <td colspan=\"${showPO ? 3 : 2}\"></td>
            </tr>`;
            detailsRows += relatedPOs.map(po => `
                <tr class=\"bg-gray-50 dark:bg-gray-900 border-b dark:border-gray-700\">
                    <td class=\"text-center px-3 py-2 text-xs text-gray-500\">${po.po_number || ''}</td>
                    <td class=\"text-center px-3 py-2 text-xs text-gray-500\">${po.po_date || ''}</td>
                    <td class=\"text-center px-3 py-2 text-xs text-gray-500\">${po.supplier || ''}</td>
                    <td class=\"text-right px-3 py-2 text-xs text-gray-500\">${formatCurrency(po.po_amount)}</td>
                    <td colspan=\"${showPO ? 3 : 2}\"></td>
                </tr>
            `).join('');
        }
        detailsRows += '</tbody>';
        return oaRow + detailsRows;
    }).join('')}
    <tr class="bg-gray-100 dark:bg-gray-900 font-semibold">
        ...
        <td class="text-right px-3 py-2">${formatCurrency(obligation_amounts.reduce((sum, r) => sum + parseFloat(r.obr_amount || 0) + parseFloat(r.adjustments || 0), 0))}</td>
                        ...
                        <table class="w-full text-xs text-center border-t mt-3 dark:text-gray-400">
                            ...
                            <tbody>${
                                obligation_adjustments.length
                                    ? buildRows(obligation_adjustments, adjTableFields)
                                ...
                                ${
                                    ...
                                    ? (() => {
                                        ...
                                        return buildRows(purchase_orders, [
                                            ...
                                            { class: 'px-2 py-2', render: r => r.programs },
                                            { class: 'px-2 py-2', render: r => r.account_code },
                                            { class: 'px-2 py-2', render: r => r.description },
                                            { class: 'px-2 py-2', render: r => r.delivery_period ?? '—' },
                                            {
