        ...
        <div class="p-6 bg-white rounded-md border-b border-gray-200 relative overflow-x-auto shadow-md sm:rounded-lg dark:bg-gray-800 dark:border-gray-700">
            <div class="flex flex-col lg:flex-row lg:justify-between lg:items-center mb-4 gap-4">
                ...
                <div class="flex flex-col sm:flex-row sm:items-center gap-2 w-full sm:w-auto">
                    <x-form.input type="text" name="search" id="searchInput" value="{{ request('search') }}" autocomplete="off" placeholder="Search for programs" class="border border-gray-300 w-full sm:w-[300px] min-w-0 rounded-lg px-4 py-2 text-xs dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white" />
            ...
            <div class="mt-4" id="programsPagination">
                @if ($perPage != 'all')
...
<script>
    ...
    function loadPrograms(url, pushHistory = true) {
        fetch(url, { headers: { 'X-Requested-With': 'XMLHttpRequest' } })
            .then(response => response.text())
            .then(html => {
                const doc = new DOMParser().parseFromString(html, 'text/html');
                ['programsTableBody', 'programsPagination'].forEach(id => {
            ...
            .catch(error => {
                console.error('Failed to load programs, falling back to full page navigation:', error);
    ...
    @if($programsLoading ?? false)
        ...
        document.addEventListener('DOMContentLoaded', function() {
            loadPrograms(window.location.href, false);
        });
    @endif
