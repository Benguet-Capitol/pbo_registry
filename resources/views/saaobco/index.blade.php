<x-app-layout>
    ...
    <form method="GET" action="" class="bg-white overflow-hidden shadow-md sm:rounded-lg mb-6 dark:bg-gray-800 transition-all duration-300 ease-in-out" id="filterForm">
        ...
        <div class="p-4 bg-white rounded-md border-b border-gray-200 relative overflow-x-auto shadow-md sm:rounded-lg dark:bg-gray-800 dark:border-gray-700 transition-colors duration-300 ease-in-out">
            <div id="saaobcoResultsContainer">
            @if($saaobcoLoading ?? false)
            <div class="flex items-center justify-center gap-2 text-gray-500 dark:text-gray-400 text-sm py-16">
                <i class="fas fa-spinner fa-spin"></i> Loading SAAOBCO data...
            </div>
            ...
            <div class="flex justify-between items-center mb-4">
                <table id="SAAOBCOTable" class="w-full text-[11px] text-gray-900 dark:text-gray-300 text-left">
                     <thead class="sticky top-0 z-10 border border-blue-400 bg-gradient-to-r from-blue-200 to-blue-300 text-blue-900 dark:border-blue-300 dark:bg-gradient-to-r dark:from-blue-700 dark:to-blue-600 dark:text-blue-200 transition-colors duration-300 ease-in-out">
                        <tr>
                            <th rowspan="2" class="px-1 py-1 w-[200px] text-center align-middle border-r border-blue-400 dark:border-blue-300">Functions / Programs / Projects / Activities</th>
                            <th rowspan="2" class="px-1 py-1 w-[80px] text-center align-middle border-r border-blue-400 dark:border-blue-300">CCO Year</th>
                            <th rowspan="2" class="px-1 py-1 w-[150px] text-center align-middle border-r border-blue-400 dark:border-blue-300">Account Code</th>
                    ...
                    <tbody class="border border-gray-300 dark:border-gray-600 text-[10px]">
                        ...
                        <tr class="{{ $rowIndex % 2 === 0 ? 'bg-gray-50 dark:bg-gray-800/60' : 'bg-white dark:bg-gray-800' }} border-b border-gray-200 dark:border-gray-700 hover:bg-blue-50 dark:hover:bg-gray-700 transition-colors duration-150 ease-in-out">
                            ...
                            <td class="px-2 py-2 font-semibold text-center">{{ $appropriation->cco_year }}</td>
                        ...
                        <tr class="bg-gradient-to-r from-gray-300 to-gray-400 text-gray-900 dark:from-gray-700 dark:to-gray-800 dark:text-gray-100 font-bold border-t-2 border-b-2 border-gray-500 dark:border-gray-600 text-[10px] transition-colors duration-300 ease-in-out">
                            ...
                            <td class="px-2 py-2 text-center" data-key="appropriation_accomplishment">
                                @include('saaob._utilization-badge', ['pct' => $office->yearlyTotals[$year]['appropriation_accomplishment'] ?? 0])
                            </td>
                            ...
                            <td class="px-2 py-2 text-center">
                                @include('saaob._utilization-badge', ['pct' => $office->yearlyTotals[$year]['allotment_accomplishment'] ?? 0])
                            </td>
                        </tr>
                        @endforeach
                        <tr class="bg-gradient-to-r from-gray-400 to-gray-500 dark:from-gray-800 dark:to-gray-900 text-gray-900 dark:text-gray-100 font-bold border-t-4 border-gray-600 dark:border-gray-600 text-[10px] transition-colors duration-300 ease-in-out">
                            ...
                            <td class="px-2 py-2 text-center" data-key="appropriation_accomplishment">
                                @include('saaob._utilization-badge', ['pct' => $office->grandTotal['appropriation_accomplishment'] ?? 0])
                            </td>
                            ...
                            <td class="px-2 py-2 text-center">
                                @include('saaob._utilization-badge', ['pct' => $office->grandTotal['allotment_accomplishment'] ?? 0])
                            </td>
                        </tr>
                        @endforeach
                        ...
                        <tr class="bg-blue-200 dark:bg-blue-950 text-blue-900 dark:text-blue-100 font-bold border-t-4 border-blue-500 dark:border-blue-800 border-b-2 text-[11px]">
                            ...
                            <td class="px-2 py-3 text-center" data-key="appropriation_accomplishment">
                                @include('saaob._utilization-badge', ['pct' => $overallTotal['appropriation_accomplishment'] ?? 0])
                            </td>
                            ...
                            <td class="px-2 py-3 text-center">
                                @include('saaob._utilization-badge', ['pct' => $overallTotal['allotment_accomplishment'] ?? 0])
                            </td>
                        </tr>
                        @endif
                    </tbody>
        ...
        <script>
            ...
            function loadSaaobco(url, pushHistory = true) {
                showLoadingOverlay();
                fetch(url, { headers: { 'X-Requested-With': 'XMLHttpRequest' } })
                    .then(response => response.text())
                    .then(html => {
                        const doc = new DOMParser().parseFromString(html, 'text/html');
                        const newEl = doc.getElementById('saaobcoResultsContainer');
                        const currentEl = document.getElementById('saaobcoResultsContainer');
                    ...
                    .catch(error => {
                        console.error('Failed to load SAAOBCO data, falling back to full page navigation:', error);
            ...
            document.getElementById('filterForm').addEventListener('submit', function(e) {
                ...
                loadSaaobco(window.location.pathname + '?' + params.toString());
            });

            // Intercept PDF generation
            window.printSAAOBCOTable = function() {
                if (!document.getElementById('SAAOBCOTable')) {
