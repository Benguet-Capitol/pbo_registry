<x-app-layout>
    ...
    <form method="GET" action="" class="bg-white overflow-hidden shadow-md sm:rounded-lg mb-6 dark:bg-gray-800 transition-all duration-300 ease-in-out" id="filterForm">
        ...
        <div class="p-4 bg-white rounded-md border-b border-gray-200 relative overflow-x-auto shadow-md sm:rounded-lg dark:bg-gray-800 dark:border-gray-700 transition-colors duration-300 ease-in-out">
            <div id="saaobgfcurrentResultsContainer">
            @if($saaobgfcurrentLoading ?? false)
            <div class="flex items-center justify-center gap-2 text-gray-500 dark:text-gray-400 text-sm py-16">
                <i class="fas fa-spinner fa-spin"></i> Loading SAAOB GF Current data...
            </div>
            @elseif($sectors->isEmpty())
    ...
    <script>
        ...
        function clearAllFilters() {
            ...
            loadSaaobgfcurrent(url.toString());
        }
        ...
        function loadSaaobgfcurrent(url, pushHistory = true) {
            showLoadingOverlay();
            fetch(url, { headers: { 'X-Requested-With': 'XMLHttpRequest' } })
                .then(response => response.text())
                .then(html => {
                    const doc = new DOMParser().parseFromString(html, 'text/html');
                    const newEl = doc.getElementById('saaobgfcurrentResultsContainer');
                    const currentEl = document.getElementById('saaobgfcurrentResultsContainer');
                ...
                .catch(error => {
                    console.error('Failed to load SAAOB GF Current data, falling back to full page navigation:', error);
        ...
        document.getElementById('filterForm').addEventListener('submit', function(e) {
            ...
            loadSaaobgfcurrent(window.location.pathname + '?' + params.toString());
        });
        ...
        window.printSAAOBGFCurrentTable = function() {
            if (!document.getElementById('saaobGFCurrentTable')) {
