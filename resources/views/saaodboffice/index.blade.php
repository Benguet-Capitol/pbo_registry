<x-app-layout>
    ...
    <form method="GET" action="" class="bg-white overflow-hidden shadow-md sm:rounded-lg mb-6 dark:bg-gray-800 transition-all duration-300 ease-in-out" id="filterForm">
        ...
        <div class="p-4 bg-white rounded-md relative dark:bg-gray-800 transition-colors duration-300 ease-in-out">
            <div id="saaodbofficeResultsContainer">
            @if($saaodbofficeLoading ?? false)
            <div class="flex items-center justify-center gap-2 text-gray-500 dark:text-gray-400 text-sm py-16">
                <i class="fas fa-spinner fa-spin"></i> Loading SAAODB Office data...
            </div>
            ...
            <div class="max-h-[700px] overflow-y-auto">
                <table id="dashboardTable" class="w-full text-[11px] text-gray-900 dark:text-gray-300 text-left">
                    <thead class="sticky top-0 z-10 border border-blue-400 bg-gradient-to-r from-blue-200 to-blue-300 text-blue-900 dark:bg-gradient-to-r dark:from-blue-900 dark:to-blue-950 dark:text-blue-200 transition-colors duration-300 ease-in-out">
                        <tr>
                            <th rowspan="2" class="px-1 py-1 w-[150px] text-center align-middle border-r border-blue-400 dark:border-blue-300" data-key="ppas">Functions / Programs / Projects / Activities</th>
                            <th rowspan="2" class="px-1 py-1 w-[100px] text-center align-middle border-r border-blue-400 dark:border-blue-300" data-key="account_code">Account Code</th>
                            ...
                            <th rowspan="2" class="px-1 py-1 w-[100px] text-center align-middle border-r border-blue-400 dark:border-blue-300" data-key="disbursement">Disbursements</th>
                        ...
                        <tr class="text-[10px]">
                            ...
                            <th class="px-1 py-1 w-[70px] text-center border-r border-blue-400 dark:border-blue-300" data-key="appropriation_accomplishment">% Obligated</th>
...
<script>
    ...
    function loadSaaodboffice(url, pushHistory = true) {
        showLoadingOverlay();
        fetch(url, { headers: { 'X-Requested-With': 'XMLHttpRequest' } })
            .then(response => response.text())
            .then(html => {
                const doc = new DOMParser().parseFromString(html, 'text/html');
                const newEl = doc.getElementById('saaodbofficeResultsContainer');
                const currentEl = document.getElementById('saaodbofficeResultsContainer');
            ...
            .catch(error => {
                console.error('Failed to load SAAODB Office data, falling back to full page navigation:', error);
    ...
    document.getElementById('filterForm').addEventListener('submit', function(e) {
        ...
        loadSaaodboffice(window.location.pathname + '?' + params.toString());
    });
