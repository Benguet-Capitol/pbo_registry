        ...
        <div class="p-6 bg-white rounded-md border-b border-gray-200 relative overflow-x-auto shadow-md sm:rounded-lg dark:bg-gray-800 dark:border-gray-700">
            <div class="flex flex-col lg:flex-row lg:justify-between lg:items-center mb-4 gap-4">
                ...
                <div class="flex flex-col sm:flex-row sm:items-center gap-2 w-full sm:w-auto">
                    <x-form.input type="text" name="search" id="searchInput" value="{{ request('search') }}" autocomplete="off" placeholder="Search for sectors" class="border border-gray-300 w-full sm:w-[300px] min-w-0 rounded-lg px-4 py-2 text-xs dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white" />
                    <form method="GET" action="{{ route('sectors.index') }}" class="w-full sm:w-auto">
            ...
            <div class="mt-4" id="sectorsPagination">
                @if ($perPage != 'all')
...
<script>
    ...
    function loadSectors(url, pushHistory = true) {
        fetch(url, { headers: { 'X-Requested-With': 'XMLHttpRequest' } })
            .then(response => response.text())
            .then(html => {
                const doc = new DOMParser().parseFromString(html, 'text/html');
                ['sectorsTableBody', 'sectorsPagination'].forEach(id => {
            ...
            .catch(error => {
                console.error('Failed to load sectors, falling back to full page navigation:', error);
    ...
    @if($sectorsLoading ?? false)
        ...
        document.addEventListener('DOMContentLoaded', function() {
            loadSectors(window.location.href, false);
        });
    @endif

    // Delegated click handler for pagination links (they get replaced on every load).
    document.addEventListener('click', function(e) {
        const link = e.target.closest('#sectorsPagination a');
        if (!link) return;
        e.preventDefault();
        loadSectors(link.href);
    });
</script>
