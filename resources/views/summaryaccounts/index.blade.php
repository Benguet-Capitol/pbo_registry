<x-app-layout>
    ...
    <form method="GET" action="" class="bg-white overflow-hidden shadow-md sm:rounded-lg mb-6 dark:bg-gray-800 transition-all duration-300 ease-in-out" id="filterForm">
        ...
        <div class="p-4 bg-white rounded-md relative dark:bg-gray-800 transition-colors duration-300 ease-in-out">
            <div id="accountsSummaryResultsContainer">
            @if($accountsSummaryLoading ?? false)
            <div class="flex items-center justify-center gap-2 text-gray-500 dark:text-gray-400 text-sm py-16">
                <i class="fas fa-spinner fa-spin"></i> Loading Accounts Summary data...
            </div>
            @elseif($allotmentClassTotals->isEmpty())
...
<script>
    ...
    function clearAllFilters() {
        ...
        loadAccountsSummary(url.toString());
    }
    ...
    function loadAccountsSummary(url, pushHistory = true) {
        showLoadingOverlay();
        fetch(url, { headers: { 'X-Requested-With': 'XMLHttpRequest' } })
            .then(response => response.text())
            .then(html => {
                const doc = new DOMParser().parseFromString(html, 'text/html');
                const newEl = doc.getElementById('accountsSummaryResultsContainer');
                const currentEl = document.getElementById('accountsSummaryResultsContainer');
            ...
            .catch(error => {
                console.error('Failed to load Accounts Summary data, falling back to full page navigation:', error);
    ...
    document.getElementById('filterForm').addEventListener('submit', function(e) {
        ...
        loadAccountsSummary(window.location.pathname + '?' + params.toString());
    });
