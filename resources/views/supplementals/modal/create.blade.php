...
<form id="createSupplementalsForm" method="POST" action="">
    ...
    <div id="createSupplementalsModal" style="display: none;" aria-hidden="true" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
        <div class="relative w-full max-w-5xl mx-4 bg-white rounded-lg shadow-lg dark:bg-gray-800 animate-scaleInUp" style="animation: scaleInUp 0.3s ease-out;">
            ...
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200 rounded-t-lg bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-gray-700 dark:to-gray-600 dark:border-gray-600">
                <h3 id="createSupplementalLabel" class="text-base font-semibold text-gray-900 dark:text-white flex items-center gap-2">
                    <i class="fas fa-plus-circle text-blue-600 dark:text-blue-400"></i>
                    {{ __('Create Supplemental Appropriation | Reversion') }}
                </h3>
                <button type="button" onclick="closeCreateSupplementalsModal()" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition-colors">
            ...
            <div class="px-6 py-4 overflow-y-auto" style="max-height: calc(90vh - 200px);">
                        ...
                        <div class="grid grid-cols-1 gap-x-6 gap-y-3 sm:grid-cols-6">
                            ...
                            <div class="sm:col-span-3 relative">
                                ...
                                <div class="mt-2">
                                    ...
                                    <div id="OfficeAllotmentClassDropdown" class="absolute w-full text-xs bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-md shadow-lg hidden max-h-48 overflow-auto z-50">
                                        <!-- Suggestions appear here -->
                                    </div>
                                    <span id="OfficeAllotmentClassError" class="text-red-500 text-xs"></span>
                                </div>
                            </div>
                            <!-- Type -->
                            <div class="sm:col-span-3">
                                <x-form.label for="type" class="block text-xs font-medium text-gray-900 dark:text-gray-200" :value="__('Type')" />
                                <div class="mt-2">
                                    <x-form.input-with-icon-wrapper>
                                        <x-slot name="icon">
                                            <i class="fas fa-arrow-up-right-dots"></i>
                                        </x-slot>
                                        <x-form.select withicon id="type" class="block w-full text-xs" type="text" name="type" placeholder="{{ __('Type') }}">
                            ...
                            <!-- Date -->
                            <div class="sm:col-span-3">
                                <x-form.label for="supplemental_date" class="block text-xs font-medium text-gray-900 dark:text-gray-200" :value="__('Date')" />
                                <div class="mt-2">
                                    <x-form.input-with-icon-wrapper>
                                        <x-slot name="icon">
                                            <i class="fas fa-calendar"></i>
                                        </x-slot>
                                        <x-form.input withicon type='date' name="supplemental_date" autocomplete="off" id="supplemental_date" placeholder="{{ __('Date') }}" :value="now()->format('Y-m-d')" max="{{ now()->format('Y-m-d') }}" class="block w-full text-xs dark:bg-gray-800 dark:text-gray-200" />
                                    </x-form.input-with-icon-wrapper>
                                </div>
                            </div>
                            <!-- Number -->
                            <div class="sm:col-span-3">
                                <x-form.label for="supplemental_no" class="block text-xs font-medium text-gray-900 dark:text-gray-200" :value="__('Number')" />
                                <div class="mt-2">
                                    <x-form.input-with-icon-wrapper>
                                        <x-slot name="icon">
                                            <i class="fas fa-list-ol"></i>
                                        </x-slot>
                                        <x-form.input withicon type='text' name="supplemental_no" autocomplete="off" id="supplemental_no" placeholder="{{ __('Number') }}" class="block w-full text-xs dark:bg-gray-800 dark:text-gray-200" />
                                    </x-form.input-with-icon-wrapper>
                                </div>
                            </div>
                            <!-- SB No. -->
                            <div class="sm:col-span-3" id="sbNoField" style="display:none;">
                                <x-form.label for="basis_no" class="block text-xs font-medium text-gray-900 dark:text-gray-200" :value="__('SB No.')" />
                                <div class="mt-2">
                                    <x-form.input-with-icon-wrapper>
                                        <x-slot name="icon">
                                            <i class="fas fa-align-justify"></i>
                                        </x-slot>
                                        <x-form.input withicon name="basis_no" autocomplete="off" id="basis_no" placeholder="{{ __('SB No.') }}" :value="old('basis_no')" class="block w-full text-xs dark:bg-gray-800 dark:text-gray-200" />
                                    </x-form.input-with-icon-wrapper>
                                    <span id="basis_noError" class="text-red-500 text-xs"></span>
                                </div>
                            </div>
                            <!-- Basis / Remarks -->
                            <div class="sm:col-span-6">
                                <x-form.label for="basis" class="block text-xs font-medium text-gray-900 dark:text-gray-200" :value="__('Basis / Remarks')" />
                                <div class="mt-2">
                                    <x-form.input-with-icon-wrapper>
                                        <x-slot name="icon">
                                            <i class="fas fa-circle-info"></i>
                                        </x-slot>
                                        <x-form.input withicon name="basis" autocomplete="off" id="basis" placeholder="{{ __('Basis / Remarks') }}" :value="old('basis')" class="block w-full text-xs dark:bg-gray-800 dark:text-gray-200" />
                                    </x-form.input-with-icon-wrapper>
                ...
                <div class="justify-center items-center p-6 flex items-center gap-3 border-t-2 border-gray-200 rounded-b-lg dark:border-gray-600 bg-gray-50 dark:bg-gray-800 flex-shrink-0">
                    ...
                    <button type="button" onclick="if(!isSubmittingSupplemental) validateSupplementalForm(); return false;" class="text-blue-600 inline-flex leading-4 tracking-wider items-center hover:text-white border border-blue-600 hover:bg-blue-600 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-xs px-6 py-2 text-center dark:border-blue-500 dark:text-blue-500 dark:hover:text-white dark:hover:bg-blue-600 dark:focus:ring-blue-900 transition-all duration-200 ease-in-out transform hover:scale-105 active:scale-95">
...
<script>
    let isSubmittingSupplemental = false;

    //Open Create Modal
    function openCreateSupplementalsModal() {
        closeAllDropdowns();
        isSubmittingSupplemental = false;
        const modal = document.getElementById('createSupplementalsModal');
        modal.style.display = 'flex';
        modal.setAttribute('aria-hidden', 'false');
        const innerDiv = modal.querySelector('div.relative');
        if (innerDiv) innerDiv.focus();
    }
    //Close Create Modal
    ...
    function validateSupplementalForm() {
        if (isSubmittingSupplemental) return false;
